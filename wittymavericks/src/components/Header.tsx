...
const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
  <Link
    href={href}
    className="text-gray-300 hover:text-[#fde047] transition-colors duration-200 text-xs font-bold tracking-widest font-mono uppercase"
  >
    {children}
  </Link>
);
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${inter.className}
        ${isScrolled ? 'bg-black/90 backdrop-blur-md shadow-lg border-b border-yellow-400/20' : 'bg-transparent'}`}
    >
      <nav className="max-w-[1440px] w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex flex-col items-center justify-center h-full space-y-8 text-sm font-bold tracking-widest font-mono">
            <Link
              href="/"
